...
/**
 * Use a servlet filter to ensure that the person viewing the app has a
 * google.com account.
 *
 * @author mimming
 */
public class AuthFilter implements Filter {
  ...
  public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain)
    ...
    if (response instanceof HttpServletResponse && request instanceof HttpServletRequest) {
      ...
      // Redirect to https when on appengine since subscriptions only work over https
      if(httpRequest.getServerName().contains("appspot.com") &&
          httpRequest.getScheme().equals("http")) {

        httpResponse.sendRedirect(
            httpRequest.getRequestURL().toString().replaceFirst("http", "https"));
        return;
      }
